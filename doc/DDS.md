...
调试发现方波 LUT 从 64 到 192 的单步跳变（Δ=128 LSBs）导致 DAC0832 的 MSB（bit 7）与其他位开关不同步，在上升沿产生短暂的中间码值毛刺，经运放后表现为尖端过冲，所以我们在每个跳变沿插入一个（或多个）中间电平采样点（128），将单步 128 LSB 跳变拆成两步 64 LSB 跳变（64→128→192），避开 MSB 单独翻转的最坏情况，glitch 能量减半
...
| 变化         | 时域现象（示波器）                                           | 频域现象（频谱）                                     |
| ------------ | ------------------------------------------------------------ | ---------------------------------------------------- |
| **m 增大 →** | 包络起伏**变大**，波形的"胖瘦"变化更剧烈，最大幅度 = A_c(1+m)，最小幅度 = A_c(1-m) | 上下边频分量幅度**增大**（边频 = m·A_c/2），载波不变 |
| **m 减小 →** | 包络起伏**变小**，趋近等幅正弦波                             | 边频幅度**减小**，趋近纯载波单根谱线                 |
| **m = 0**    | 纯载波，无调制                                               | 只有载波频率 f_c 一根谱线                            |
| **m = 1**    | 100% 调制，包络最低点刚好触零                                | 边频幅度 = 载波一半                                  |
| **m > 1**    | 过调制，包络变形、相位反转（代码中 `MOD_INDEX_MAX` 限制防止了这种情况） | —                                                    |
...
| 变化         | 时域现象（示波器）                                           | 频域现象（频谱）                                             |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **β 增大 →** | 波形**疏密变化剧烈**——频率在更宽范围内摆动（Δf = β·f_m 更大），但**幅度恒定不变** | 频谱**展宽**，显著边带对数增多（Bessel 函数 J_n(β)），带宽 ≈ 2(β+1)·f_m（Carson 规则） |
| **β 减小 →** | 疏密变化**减弱**，趋近恒定频率的正弦波                       | 频谱**收窄**，边带减少，趋近单根载波谱线                     |
| **β < 0.5**  | 几乎看不出频率变化                                           | 窄带 FM，只有一对显著边带（类似 AM）                         |
| **β 大**     | 频率摆动很明显，波形在示波器上呈现"一段密一段疏"             | 宽带 FM，多条边带                                            |
...
- 把 `f_dac` 改成1.00us，极限增大更新率保证SPWM方波边缘，示波器测得占空比从63.805%降到47.857%（m_a = 80%），但是仍有斜边

| 变化           | 时域现象（示波器）                                           | 频域现象（频谱）                                             |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **m_a 增大 →** | 脉冲**宽度正弦变化更明显**——宽的地方更宽、窄的地方更窄。低通滤波后基波**幅度线性增大** | 基波分量（f_m）**幅度增大**，载波谐波和边带谐波分布变化（由 J_k(n·π·m_a/2) 决定） |
| **m_a 减小 →** | 占空比变化范围**缩小**，趋近 50% 固定占空比方波。滤波后正弦波幅度**减小** | 基波分量**减小**                                             |
| **m_a = 0**    | 固定 50% 方波输出（高低交替）                                | 无基波分量                                                   |
| **m_a = 1**    | 最大线性调制，占空比从接近 0% 到接近 100% 正弦变化           | 基波幅度达到最大（= V_dc/2）                                 |
| **m_a > 1**    | 过调制，出现脉冲丢失/饱和，谐波增加                          | 线性关系破坏，低次谐波增加                                   |
